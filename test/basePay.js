...
describe("seperate BasePay to object",function(){
  let eslistmentDay = new Date("2020","0","20");
  let salaryDateInfomation = {
    year: 2021,
    month : 3
  }
    it("2021년 3월 바뀐 기본급 계산 ",function(){
      let servicedMonth = calculateServiceMonth(eslistmentDay,salaryDateInfomation);
      let userClass = returnClass(servicedMonth);
      let base = basepay[salaryDateInfomation.year][userClass];

      expect(base).eq(608500);
    })
})